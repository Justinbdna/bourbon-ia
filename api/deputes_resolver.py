...
def _load_deputes_db() -> dict[str, dict[str, str]]:
    """Charge le snapshot local des députés (singleton en mémoire)."""
    global _DEPUTES_DB
    if _DEPUTES_DB is not None:
        return _DEPUTES_DB

    json_path = os.path.join(os.path.dirname(__file__), "data", "deputes_active.json")
    if os.path.exists(json_path):
        try:
            with open(json_path, "r", encoding="utf-8") as f:
                _DEPUTES_DB = json.load(f)
                logger.info(f"🏛️ Référentiel députés chargé : {len(_DEPUTES_DB)} entrées.")
        except Exception as e:
            logger.warning(f"⚠️ Impossible de charger deputes_active.json : {e}")
            _DEPUTES_DB = {}
    else:
